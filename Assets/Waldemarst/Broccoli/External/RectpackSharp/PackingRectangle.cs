...
namespace Broccoli.RectpackSharp
{
    ...
    public struct PackingRectangle : IEquatable<PackingRectangle>, IComparable<PackingRectangle>
    {
        ...
        /// <summary>
        /// A value used to group rectangles according to their origin.
        /// </summary>
        public int Group;
        ...
        /// <summary>The padding to apply of this <see cref="PackingRectangle"/>.</summary>
        public uint Padding;

        /// <summary>The X coordinate of the left edge of this <see cref="PackingRectangle"/> with padding applied.</summary>
        public uint PX
        {
            get => (X>0?(Width-Padding<1?X:X+Padding):0);
        }

        /// <summary>The Y coordinate of the top edge of this <see cref="PackingRectangle"/> with padding applied.</summary>
        public uint PY
        {
            get => (Y>0?(Height-Padding<1?Y:Y+Padding):0);
        }

        /// <summary>The width of this <see cref="PackingRectangle"/> with padding applied.</summary>
        public uint GetPWidth (uint widthLimit)
        {
            if (Right >= widthLimit) {
                return Width;
        ...
        /// <summary>The height of this <see cref="PackingRectangle"/> with padding applied.</summary>
        public uint GetPHeight (uint heightLimit)
        {
            if (Bottom >= heightLimit) {
                return Height;
            } else if (Height-Padding<1) {
                return Height;
            } else {
        ...
        /// </summary>
        public PackingRectangle(uint x, uint y, uint width, uint height, int id = 0, int group = 0)
        {
            X = x;
            Y = y;
            Width = width;
        ...
        {
            if (hashCode < 0) {
                string hash = string.Format ("{0}_{1}_{2}_{3}_{4}", X, Y, Width, Height, Id);
