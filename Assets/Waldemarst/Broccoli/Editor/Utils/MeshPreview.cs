				...
				if (hasSecondPass) {
					if (pass == 1) {
						...
						if (firstPassTex == null) {
							...
							firstPassTex = RenderTexture.GetTemporary (resultRender.width, resultRender.height, 16, format);
							...
							/*
							GraphicsFormat format = camera.allowHDR ? GraphicsFormat.R16G16B16A16_SFloat : GraphicsFormat.R8G8B8A8_UNorm;
							firstPassTex = new RenderTexture(resultRender.width, resultRender.height, format, SystemInfo.GetGraphicsFormat(DefaultFormat.DepthStencil));
							firstPassTex.hideFlags = HideFlags.HideAndDontSave;
							*/
		...
		void OnPostRender (Camera cam)
		{
			if (cam == _previewRenderUtility.camera) {
				if (hasSecondPass && currentPass == 2) {
					...
					if (blendTex == null) {
						GraphicsFormat format = camera.allowHDR ? GraphicsFormat.R16G16B16A16_SFloat : GraphicsFormat.R8G8B8A8_UNorm;
						blendTex = RenderTexture.GetTemporary (firstPassTex.width, firstPassTex.height, 16, format);
						//blendTex = RenderTexture.GetTemporary (firstPassTex.width, firstPassTex.height, 24, RenderTextureFormat.Default, RenderTextureReadWrite.Linear);
						/*
						GraphicsFormat format = camera.allowHDR ? GraphicsFormat.R16G16B16A16_SFloat : GraphicsFormat.R8G8B8A8_UNorm;
						firstPassTex = new RenderTexture(firstPassTex.width, firstPassTex.height, format, SystemInfo.GetGraphicsFormat(DefaultFormat.DepthStencil));
						firstPassTex.hideFlags = HideFlags.HideAndDontSave;
						*/
