	...
	public class SproutLabEditor {
		...
		*/
		...
		bool DrawSproutAStructurePanel () {
			...
			if (branchStructureLevelOptions.Length > 1) {
				selectedSnapshot.selectedLevelIndex = GUILayout.Toolbar (selectedSnapshot.selectedLevelIndex, sproutAStructureLevelOptions);
			}
			...
			if (isEnabled != proxySproutALevelDescriptor.isEnabled) {
				...
				selectedSproutALevelDescriptor.isEnabled = isEnabled;
		...
		bool DrawSproutBStructurePanel () {
			...
			if (branchStructureLevelOptions.Length > 1) {
				selectedSnapshot.selectedLevelIndex = GUILayout.Toolbar (selectedSnapshot.selectedLevelIndex, sproutBStructureLevelOptions);
			}
			...
			if (isEnabled != proxySproutBLevelDescriptor.isEnabled) {
				...
				selectedSproutBLevelDescriptor.isEnabled = isEnabled;
				UpdateStructureLevelsGUI (selectedSnapshot.activeLevels);
			}
			EditorGUILayout.Space ();
			if (isEnabled) {
		...
		private void OnSaveBranchDescriptorCollectionSO (BranchDescriptorCollectionSO branchDescriptorCollectionSO, string pathToFile) {
			Debug.Log ($"BranchDescriptorCollection saved to {pathToFile}");
		}
		#endregion
