...
namespace Broccoli.TreeNodeEditor
{
	...
	public class LODListComponent {
		...
		/// </summary>
		float billboardPercentage = 0.4f;
		/// <summary>
		...
		#region GUI
		private static GUIContent includeInPrefabLabel = new GUIContent ("Include in Prefab", "If checked this LOD will be included in the final Prefab Asset.");
		private static GUIContent minPolygonSidesLabel = new GUIContent ("Min Polygon Sides", "Minimum number of sides on the polygon used to create the branches mesh.");
		private static GUIContent maxPolygonSidesLabel = new GUIContent ("Max Polygon Sides", "Maximum number of sides on the polygon used to create the branches mesh.");
		private static GUIContent branchAngleToleranceAtBaseLabel = new GUIContent ("Angle Tolerance at Base", 
			"Resolution used to process branch curves to create segments at the base of the tree. The higher the angle, the lesser the resolution.");
		private static GUIContent branchAngleToleranceAtTopLabel = new GUIContent ("Angle Tolerance at Top", 
			"Resolution used to process branch curves to create segments at the top of the tree. The higher the angle, the lesser the resolution.");
		private static GUIContent useMeshCapAtBaseLabel = new GUIContent ("Use Mesh Cap at Base", "Add triangles to the base of each branch to conceal inner (non-rendereable) planes.");
		private static GUIContent allowBranchWeldingLabel = new GUIContent ("Allow Branch Welding", "If unchecked then branch welding is disabled on the tree. " +
			"Branch welding adds geometry at the base of the branch to smooth transition the parent branch mesh to the child branch mesh.");
		private static GUIContent allowRootWeldingLabel = new GUIContent ("Allow Root Welding", "If unchecked then root welding is disabled on the tree. " +
			"Root welding adds geometry at the base of the root to smooth transition the parent root mesh to the child root mesh.");
		private static GUIContent sproutResolutionLabel = new GUIContent ("Sprout Resolution", "Sets the level the geometry on the sprouts to use on this LOD. " +
			"A value of 0 produces the most basic geometry for sprouts, while a value of 1 produces more complex geometry (to use on LOD0 for example).");
		private static GUIContent groupPercentageLabel = new GUIContent ("LOD Group %", "Pencentage of of this LOD level for the transitions on the LOD Group.");
		private static GUIContent setAsPreviewLODLabel = new GUIContent ("Set this LOD As Preview", "Sets the selected LOD definition as default to use as preview in the editor and at runtime.");
		...
		static string MSG_INCLUDE_IN_PREFAB = "If checked this LOD will be included in the final Prefab Asset.";
		static string MSG_MIN_POLYGON_SIDES = "Minimum number of sides on the polygon used to create the branches mesh.";
		static string MSG_MAX_POLYGON_SIDES = "Maximum number of sides on the polygon used to create the branches mesh.";
		static string MSG_BRANCH_ANGLE_AT_BASE_TOLERANCE = "Resolution used to process branch curves to create segments at the base of the tree. The higher the angle, the lesser the resolution.";
		static string MSG_BRANCH_ANGLE_AT_TOP_TOLERANCE = "Resolution used to process branch curves to create segments at the top of the tree. The higher the angle, the lesser the resolution.";
		...
		static string MSG_SPROUT_RESOLUTION = "Sets the level the geometry on the sprouts to use on this LOD. " +
			"A value of 0 produces the most basic geometry for sprouts, while a value of 1 produces more complex geometry (to use on LOD0 for example).";
		...
		void DrawItem (Rect rect, int index, bool isActive, bool isFocused) {
			...
			if (lod != null) {
				...
				GUI.Label (new Rect (rect.x, rect.y, 220, EditorGUIUtility.singleLineHeight + 5), 
					"LOD " + index + " (" + lod.preset + (lod.hasCustomValues?", modified":"") +")" + (index == previewLODIndex?" (Preview)":""));
				if (isActive) {
					EditorGUILayout.LabelField ("LOD " + index + " (" + lod.preset + ")", 
						BroccoEditorGUI.labelBoldCentered);
					// INCLUDE IN PREFAB.
					bool includeInPrefab = EditorGUILayout.Toggle (includeInPrefabLabel, lod.includeInPrefab);
					...
					EditorGUILayout.Space ();
					EditorGUILayout.LabelField ("Branches", BroccoEditorGUI.labelBold);
					// MIN POLYGON SIDES.
					int minPolygonSides = EditorGUILayout.IntSlider (minPolygonSidesLabel, lod.minPolygonSides, 3, lod.maxPolygonSides);
					ShowHelpBox (MSG_MIN_POLYGON_SIDES);
					if (minPolygonSides != lod.minPolygonSides) {
						onBeforeEditLOD?.Invoke (lod);
						lod.minPolygonSides = minPolygonSides;
						lod.hasCustomValues = true;
						onEditLOD?.Invoke (lod);
						if (lod == previewLOD)
							onRequiresRebuild?.Invoke ();
					}
					// MAX POLYGON SIDES.
					int maxPolygonSides = EditorGUILayout.IntSlider (maxPolygonSidesLabel, lod.maxPolygonSides, lod.minPolygonSides, 20);
					...
					// BRANCH ANGLE TOLERANCE AT BASE.
					float branchAngleToleranceAtBase = EditorGUILayout.Slider (branchAngleToleranceAtBaseLabel, lod.branchAngleToleranceAtBase, 5, 90);
					ShowHelpBox (MSG_BRANCH_ANGLE_AT_BASE_TOLERANCE);
					if (branchAngleToleranceAtBase != lod.branchAngleToleranceAtBase) {
						onBeforeEditLOD?.Invoke (lod);
						lod.branchAngleToleranceAtBase = branchAngleToleranceAtBase;
						lod.hasCustomValues = true;
						onEditLOD?.Invoke (lod);
						if (lod == previewLOD)
							onRequiresRebuild?.Invoke ();
					}
					// BRANCH ANGLE TOLERANCE AT TOP.
					float branchAngleToleranceAtTop = EditorGUILayout.Slider (branchAngleToleranceAtTopLabel, lod.branchAngleToleranceAtTop, 5, 90);
					ShowHelpBox (MSG_BRANCH_ANGLE_AT_TOP_TOLERANCE);
					if (branchAngleToleranceAtTop != lod.branchAngleToleranceAtTop) {
						onBeforeEditLOD?.Invoke (lod);
						lod.branchAngleToleranceAtTop = branchAngleToleranceAtTop;
						lod.hasCustomValues = true;
						onEditLOD?.Invoke (lod);
						if (lod == previewLOD)
							onRequiresRebuild?.Invoke ();
					...
					bool useMeshCapAtBase = EditorGUILayout.Toggle (useMeshCapAtBaseLabel, lod.useMeshCapAtBase);
					...
					bool allowBranchWelding = EditorGUILayout.Toggle (allowBranchWeldingLabel, lod.allowBranchWelding);
					...
					// ALLOW ROOT WELDING.
					bool allowRootWelding = EditorGUILayout.Toggle (allowRootWeldingLabel, lod.allowRootWelding);
					...
					EditorGUILayout.Space ();
					EditorGUILayout.LabelField ("Sprouts", BroccoEditorGUI.labelBold);
					// SPROUT RESOLUTION.
					float sproutResolution = EditorGUILayout.Slider (sproutResolutionLabel, lod.sproutResolution, 0f, 1f);
					ShowHelpBox (MSG_SPROUT_RESOLUTION);
					if (sproutResolution != lod.sproutResolution) {
						onBeforeEditLOD?.Invoke (lod);
						lod.sproutResolution = sproutResolution;
						lod.hasCustomValues = true;
						onEditLOD?.Invoke (lod);
						if (lod == previewLOD)
							onRequiresRebuild?.Invoke ();
					...
					EditorGUILayout.Space ();
					EditorGUILayout.LabelField ("LOD Groups", BroccoEditorGUI.labelBold);
					// LOD GROUP PERCENTAGE.
					float groupPercentage = EditorGUILayout.Slider (groupPercentageLabel, lod.groupPercentage * 100f, 5f, 100f);
					...
					if (previewLODIndex != index) {
						if (GUILayout.Button (setAsPreviewLODLabel)) {
							previewLOD = lod;
