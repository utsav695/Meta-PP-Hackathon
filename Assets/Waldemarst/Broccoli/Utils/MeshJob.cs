...
namespace Broccoli.Utils
{
    ...
    public class MeshJob {
		...
		protected List<Vector3> _anchors = new List<Vector3> ();
		...
		protected List<Vector4> uv3s = new List<Vector4> ();
		...
		struct MeshJobImpl : IJobParallelFor {
			...
			/// <summary>
			/// Contains the ANCHOR (x, y, z).
			/// </summary>
			public NativeArray<Vector3> anchor;
			/// <summary>
			...
			public static int APPLY_UV3_ANCHOR = 128;
			public static int APPLY_UV5_TRANSFORM = 256;
			public static int APPLY_UV6_TRANSFORM = 512;
			public static int APPLY_UV7_TRANSFORM = 1024;
			public static int APPLY_UV8_TRANSFORM = 2048;
			#endregion
			...
			public void Execute (int i) {
				...
				if ((flag & APPLY_UV3_ANCHOR) == APPLY_UV3_ANCHOR) {
					Vector3 spAnchor = (Vector3)anchor [i];
					ApplyAnchor (vertexStart, vertexEnd, spAnchor);
				}
			...
			public void ApplyAnchor (int vertexStart, int vertexEnd, Vector3 anchor) {
				for (int i = vertexStart; i < vertexEnd; i++) {
					Vector4 uv3 = uv3s[i];
					uv3.x = anchor.x;
					uv3.y = anchor.z;
		...
		public void SetAnchorPosition (Vector3 anchorPos)
		{
			...
			_anchors[lastIndex] = anchorPos;
			_flags [lastIndex] |= MeshJobImpl.APPLY_UV3_ANCHOR;
        ...
        public void ExecuteJob () {
			...
			MeshJobImpl _meshJob = new MeshJobImpl () {
				...
				anchor = new NativeArray<Vector3> (_anchors.ToArray (), Allocator.TempJob),
				...
				uv3s = new NativeArray<Vector4> (uv3s.ToArray (), Allocator.TempJob),
